...
func _process(delta):
	...
	if (target!=null):
		...
		ang = (Vector2(0,-1).rotated(get_node("../../").get_rot()+get_rot())).angle_to(pos-tpos+(get_node("../../").get_lv()+speed*dir-tlv)*d/speed)
		if (d<range_max && (abs(ang)<0.1 || abs(ang)>3.1)):
			shoot = active && !get_node("../../").boarding
			rotate = 0
		else:
			rotate = -sign(ang)
		if (d>range_max || !(target in get_node("/root/Main").ships)):
			find_target()
	else:
		find_target()
