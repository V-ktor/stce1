...
func find_nav_point():
	var pos = get_node("/root/Main/"+planet+"/Planet").get_global_pos()+Vector2(rand_range(-50000,50000),rand_range(-50000,50000))
	nav.push_back(pos)
...
func init():
	...
	var ii = Sprite.new()
	ii.set_texture(load("res://images/icons/icon_police.png"))
	...
	var pos = get_global_pos()
	if (pos.distance_squared_to(get_node("/root/Main/Planet3/Planet").get_global_pos())<pos.distance_squared_to(get_node("/root/Main/Planet2/Planet").get_global_pos())):
		planet = "Planet3"
	else:
		planet = "Planet2"
	find_nav_point()
